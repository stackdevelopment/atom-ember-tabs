...
export default class TabWatcher {
  ...
  setBonusTitle(tab){
    ...
    let typeDiv = tab.itemTitle;
    if(typeDiv.childNodes.length === 1 ){
      let bonusTitleDiv = document.createElement('div');
      bonusTitleDiv.className = 'pod-file-type';
      var bonusTitleText = document.createTextNode(bonusTitle);
      bonusTitleDiv.appendChild(bonusTitleText); //add the text node to the newly created div.
      tab.element.insertBefore(bonusTitleDiv, tab.element.querySelector('.title')); //add the text node to the newly created div.
    }
  }
